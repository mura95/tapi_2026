...
{
    /// <summary>
    /// Wake Word検出器（Google Colabカスタムモデル対応版）
    /// 16フレーム × 96メルバンドのメルスペクトログラムを直接入力
    /// </summary>
    public class WakeWordDetector : MonoBehaviour
    {
        ...
        [SerializeField] private float bufferDuration = 1.28f;
        [SerializeField] private float detectionCooldown = 2.0f;

        [Header("Audio Settings")]
        [SerializeField] private int sampleRate = 16000;

        [Header("Mel Spectrogram Settings")]
        [SerializeField] private int nMels = 96;        // Google Colabのデフォルト
        ...
        [SerializeField] private int nFrames = 16;      // 固定16フレーム
        ...
        private void Awake()
        {
            // 必要なサンプル数 = (nFrames - 1) * hopLength + winLength
        ...
        private void InitializeModel()
        {
            ...
            if (modelAsset == null)
            {
                GameLogger.LogError(LogCategory.Voice,"[WakeWordDetector] Model Asset not assigned!");
                return;
            }
            ...
            try
            {
                ...
                IsInitialized = true;
                Log("✅ Wake Word Detector initialized");
                Log($"Backend: {backend}");
            ...
            {
                GameLogger.LogError(LogCategory.Voice,$"[WakeWordDetector] Failed to initialize: {e.Message}\n{e.StackTrace}");
                IsInitialized = false;
            }
        }
        ...
        public void ProcessAudioData(float[] audioData)
        {
            ...
            {
                GameLogger.LogWarning(LogCategory.Voice,$"[WakeWordDetector] Skip: init={IsInitialized}, data={audioData?.Length ?? 0}");
                return;
            }

            GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] Before buffer: {audioBuffer.Count}");
            ...
            GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] After buffer: {audioBuffer.Count}/{bufferSize}");

            if (audioBuffer.Count >= bufferSize)
            {
                GameLogger.Log(LogCategory.Voice,"[WakeWordDetector] Calling DetectWakeWord");
                DetectWakeWord();
            }
        }
        ...
        {
            GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] === DetectWakeWord START ===");
            GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] Buffer count: {audioBuffer.Count}, Cooldown check: {Time.time - lastDetectionTime:F2}s");

            if (Time.time - lastDetectionTime < detectionCooldown)
            {
                GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] ⏸ Cooldown active, skipping");
                return;
            }

            try
            {
                GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] Converting buffer to array...");
                float[] audioArray = audioBuffer.ToArray();
                GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] Array length: {audioArray.Length}");

                GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] Extracting mel spectrogram...");
                float[,] melSpec = ExtractMelSpectrogram(audioArray);

                if (melSpec == null)
                {
                    GameLogger.LogError(LogCategory.Voice,"[WakeWordDetector] ❌ Mel spec is NULL");
                    return;
                }

                GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] Mel spec: [{melSpec.GetLength(0)}, {melSpec.GetLength(1)}]");
                GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] Calling Predict...");

                float score = Predict(melSpec);

                GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] 📊 Score: {score:F4}");
                ...
                {
                    GameLogger.Log(LogCategory.Voice,$"[WakeWordDetector] 🎤 DETECTED! Score: {score:F3}");
            ...
            {
                GameLogger.LogError(LogCategory.Voice,$"[WakeWordDetector] ❌ Error: {e.Message}\n{e.StackTrace}");
            }
        }

        /// <summary>
        /// メルスペクトログラムを抽出（Google Colabと同じ方式）
        /// 出力: [nFrames, nMels] = [16, 96]
        /// </summary>
        private float[,] ExtractMelSpectrogram(float[] audio)
        {
            try
            {
                ...
                for (int frameIdx = 0; frameIdx < nFrames; frameIdx++)
                {
                    ...
                    for (int melIdx = 0; melIdx < nMels; melIdx++)
                    {
                        ...
                        // ログスケール変換（librosaのpower_to_dbと同様）
            ...
            {
                GameLogger.LogError(LogCategory.Voice,$"[WakeWordDetector] Mel spectrogram extraction error: {e.Message}");
                return null;
            }
        }
        ...
        /// <summary>
        /// モデル推論（Google Colabカスタムモデル用）
        /// 入力形状: (1, 16, 96)
        /// </summary>
        private float Predict(float[,] melSpec)
        {
            // 2D配列を1D配列に変換（行優先: [time, mel] → [time*mel]）
            ...
            try
            {
                ...
                {
                    GameLogger.LogError(LogCategory.Voice,"[WakeWordDetector] Output tensor is null");
                    return 0f;
                }

                float[] output = outputTensor.DownloadToArray();

                // Sigmoid適用（Google ColabはBCEWithLogitsLossで学習）
            ...
            {
                GameLogger.LogError(LogCategory.Voice,$"[WakeWordDetector] Prediction error: {e.Message}\n{e.StackTrace}");
                return 0f;
            }
            finally
            {
        ...
        {
            if (showDebugLog)
                GameLogger.Log(LogCategory.Voice, $"[WakeWordDetector] {message}");
        }
    }
}